...
class SpeedWorker(Worker):
    ...
    def calculate(self, match_data, gsd):
        ...
        for i in range(0, len(coords1)): # iterate through all the matches and calculate the average distance
            ...
            x2 = coords2[i][0]
            y2 = coords2[i][1]
            sumDistance += math.sqrt((x2-x1)**2 + (y2-y1)**2)
        return sumDistance/len(coords1)/match_data.timeDifference*gsd
        ...
        while not self.cancelled:  # loop until cancelled
            try:
                if not self.queue.empty():
                    ...
                    newSpeed = (speed*processedCount + self.calculate(matchData, self.gsd))/(processedCount+1)
                    processedCount+=1
